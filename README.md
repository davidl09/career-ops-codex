...
Career-ops is agentic: Codex runs the workflow, the checked-in modes drive the reasoning, and Playwright handles PDF generation plus reliable job verification.
...
# 2. Check setup
bun run doctor                     # Validates all prerequisites
...
> **The system is designed to be customized by Codex itself.** Modes, archetypes, scoring weights, negotiation scripts -- ask Codex to change them in-place. It reads the same files it uses, so it knows exactly what to edit.
...
- **Agent**: OpenAI Codex with custom skills and modes
